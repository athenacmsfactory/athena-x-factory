  ...
  const fetchArchive = async () => {
    ...
    try {
      ...
      const filteredData = repos
        ...
        .map(repo => {
          // Zoek een match in de lokale items voor het pakket label
          // We kijken naar de naam (met streepjes) OF de link
          const localMatch = (items || []).find(i => {
            const friendlyName = (i.name || '').toLowerCase().replace(/[\s&]+/g, '-');
            return friendlyName === repo.name.toLowerCase() || 
                   (i.link && i.link.toLowerCase().includes(repo.name.toLowerCase()));
          });
          
          let level = 'Starter';
          if (localMatch && localMatch.pakket) {
            level = localMatch.pakket;
          } else if (repo.name.toLowerCase().includes('shop') || repo.name.toLowerCase().includes('store') || repo.name.toLowerCase().includes('webshop')) {
            level = 'Professional';
          } else if (repo.name.toLowerCase().includes('-ath') || repo.name.toLowerCase().includes('factory')) {
             level = 'Professional';
          }
              ...
              <div key={index} className="group flex flex-col bg-white rounded-[2rem] overflow-hidden shadow-xl border border-slate-100 hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
                ...
                <div className="p-8 flex-1 flex flex-col">
                  ...
                  <div className="flex flex-wrap gap-3 mb-5">
                    {/* Altijd Pakket Badge Tonen (indien aanwezig in item) */}
                    {item.pakket && (
                      <span className={`text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full shadow-sm ${
                        String(item.pakket).toLowerCase().includes('starter') ? 'bg-slate-500 text-white' :
                        String(item.pakket).toLowerCase().includes('professional') ? 'bg-accent text-white' :
                        String(item.pakket).toLowerCase().includes('business') ? 'bg-yellow-500 text-black' :
                    ...
                    {/* Andere Category/Type Labels */}
                    {fieldsToRender.map((fk) => {
                        const isCategory = fk.toLowerCase().includes('cat') || fk.toLowerCase().includes('type');
                        const isPakket = fk.toLowerCase() === 'pakket';
                        if (!isCategory || isPakket) return null; // Pakket hebben we hierboven al handmatig gedaan
                        
                        const val = item[fk];
                        const displayText = typeof val === 'object' ? (val.text || val.label || val.title || JSON.stringify(val)) : val;

                        return (
                          <span key={fk} className="text-[10px] font-black uppercase tracking-[0.2em] text-accent/80 flex items-center bg-accent/5 px-3 py-1 rounded-full border border-accent/10">
                  ...
                  {fieldsToRender.map((fk) => {
                      const isCategory = fk.toLowerCase().includes('cat') || fk.toLowerCase().includes('type');
                      const isPakket = fk.toLowerCase() === 'pakket';
                      if (isCategory || isPakket) return null;

                      const val = item[fk];
                      const displayText = typeof val === 'object' ? (val.text || val.label || val.title || JSON.stringify(val)) : val;
                      
                      // Identify first non-label field as Title
                      const firstTextField = fieldsToRender.find(f => !f.toLowerCase().includes('cat') && !f.toLowerCase().includes('type') && f.toLowerCase() !== 'pakket');
                      const isTitle = fk === firstTextField;
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b-2 border-slate-50">
                        <th className="py-6 px-4 font-black uppercase tracking-widest text-[10px] text-slate-400">Pakket</th>
                        ...
                        <tr key={idx} className="group hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0">
                          <td className="py-6 px-4">
                            <span className={`px-3 py-1 rounded-full text-[9px] font-bold uppercase shadow-sm ${
                                project.level === 'Business' ? 'bg-yellow-100 text-yellow-700' : 
                                project.level === 'Professional' ? 'bg-blue-100 text-blue-700' : 
