...
export default function HeroLegoV9({ data = {}, sectionName = 'hero' }) {
  const { 
    ...
    link_knop = '#',
    afbeelding_bg = 'images/hero-bg.webp'
  } = data;

  const base = import.meta.env.BASE_URL || '/';
  const finalImage = (afbeelding_bg.startsWith('http') || afbeelding_bg.startsWith('data:')) 
    ? afbeelding_bg 
    : (base + (afbeelding_bg.startsWith('/') ? afbeelding_bg.slice(1) : afbeelding_bg)).replace(/\/+/g, '/');
