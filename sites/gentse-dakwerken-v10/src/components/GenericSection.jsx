...
const GenericSection = ({ data, sectionName, sectionTitle, layout = 'list', features = {}, style = {} }) => {
    ...
    const getImgPath = (url) => {
        ...
        // Als het al Begint met de base-path (voor docking), niet dubbel doen
        if (base !== '/' && cleanUrl.startsWith(base.slice(1))) return (base + cleanUrl.slice(base.length - 1)).replace(/\/+/g, '/');
        const pathPrefix = (cleanUrl.includes('images/') || cleanUrl.endsWith('.svg')) ? '' : 'images/';
        return (base + pathPrefix + cleanUrl).replace(/\/+/g, '/');
    };

    const title = sectionTitle || sectionName.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
            ...
            <section id={sectionName} data-dock-section={sectionName} className="py-24 px-6 bg-[var(--color-background,#1a1a2e)]" style={style}>
                <div className="max-w-7xl mx-auto">
                    <div className="flex flex-col items-center mb-16 text-center">
                        <h2 className="text-4xl md:text-5xl font-serif font-bold text-[var(--color-heading,#bc6c25)] mb-4 capitalize">{title}</h2>
