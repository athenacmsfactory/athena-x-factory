                        ...
                        return (
                            <div key={index} className="flex flex-col items-center text-center group">
                                ...
                                <h3 className="text-xl font-bold text-primary mb-1">
                                    <span data-dock-bind={JSON.stringify({ file: sectionName, index, key: naamKey })}>{item[naamKey]}</span>
                                </h3>
                                <div className="text-sm font-bold text-accent uppercase tracking-widest mb-4">
                                    <span data-dock-bind={JSON.stringify({ file: sectionName, index, key: rolKey })}>{item[rolKey]}</span>
                                </div>
                                {bioKey && (
                                    <p className="text-slate-600 text-sm leading-relaxed max-w-xs mx-auto">
                                        <span data-dock-bind={JSON.stringify({ file: sectionName, index, key: bioKey })}>{item[bioKey]}</span>
                                    </p>
                                )}
                            </div>
                        );
